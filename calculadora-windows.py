...
DOC STRING
    ...
    Se você encontrar algum problema sério na calculadora peço que entre em contato comigo pelo LinkedIn, gratidão.
...
minus = plus = equals = divide = times = accumulating = ""
    ...
    # Mouse Events (Changes the Button background color)

    # Number Buttons
def entra_mouse(event):
    event.widget.configure(bg="lightblue", fg="#091218")
def sai_mouse(event):
    event.widget.configure(bg="#091218", fg="lightblue")

    # Operator Buttons
def entra_mouse_op(event):
    event.widget.configure(bg="lightblue", fg="#284152")
def sai_mouse_op(event):
    event.widget.configure(bg="#284152", fg="lightblue")

    # Equals Button
def entra_mouse_equals(event):
    event.widget.configure(bg="lightblue", fg="black")
def sai_mouse_equals(event):
    event.widget.configure(bg="#5FCCBD", fg="black")
...
root_position_x = (screen_width / 2) - (350 / 2)
root_position_y = (screen_height / 2) -  (450 / 2)
root.geometry("%dx%d+%d+%d" % (350,450, root_position_x, root_position_y)) # Defines the 'root' size and initial position
...
botao_limpar = Button(root, text="C", bg="#901F1F",command=clear_function, font=("Arial",10), fg="white", height=1, activebackground="pink", relief="flat", highlightthickness=0, width=2)
botao_limpar.grid(row=1, column=0, sticky="ws")

botao_point = Button(root, text=".", command=lambda: char_collect(botao_point.cget('text')), bg="black", fg="lightblue", activebackground="lightblue", relief="flat", highlightthickness=0, font=("Arial", 10), height=1, width=2)
botao_point.place(x=22, y=110)
...
botao1.bind("<Enter>", entra_mouse)
botao1.bind("<Leave>", sai_mouse)
...
botao2.bind("<Enter>", entra_mouse)
botao2.bind("<Leave>", sai_mouse)
...
botao3.bind("<Enter>", entra_mouse)
botao3.bind("<Leave>", sai_mouse)
...
botao4.bind("<Enter>", entra_mouse)
botao4.bind("<Leave>", sai_mouse)
...
botao5.bind("<Enter>", entra_mouse)
botao5.bind("<Leave>", sai_mouse)
...
botao6.bind("<Enter>", entra_mouse)
botao6.bind("<Leave>", sai_mouse)
...
botao7.bind("<Enter>", entra_mouse)
botao7.bind("<Leave>", sai_mouse)
...
botao8.bind("<Enter>", entra_mouse)
botao8.bind("<Leave>", sai_mouse)
...
botao9.bind("<Enter>", entra_mouse)
botao9.bind("<Leave>", sai_mouse)
...
botao0.bind("<Enter>", entra_mouse)
botao0.bind("<Leave>", sai_mouse)
...
plus_button.bind("<Enter>", entra_mouse_op)
plus_button.bind("<Leave>", sai_mouse_op)
...
minus_button.bind("<Enter>", entra_mouse_op)
minus_button.bind("<Leave>", sai_mouse_op)
...
times_button.bind("<Enter>", entra_mouse_op)
times_button.bind("<Leave>", sai_mouse_op)
...
divide_button.bind("<Enter>", entra_mouse_op)
divide_button.bind("<Leave>", sai_mouse_op)
...
equals_button.bind("<Enter>", entra_mouse_equals)
equals_button.bind("<Leave>", sai_mouse_equals)
